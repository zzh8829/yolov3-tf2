...
def parse_bounding_box_labels(labels, classes, image_width, image_height):
    result = []
    for obj in labels:
        if 'box' in obj['shape'] and obj['class'] in classes:
            label = parse_bounding_box(obj['shape']['box'], image_width, image_height)
            label['classname'] = obj['class']
            label['classid'] = classes.index(obj['class'])
            label['difficulty'] = 0
            label['truncated'] = 0
            label['view'] = ""
            result.append(label)
...
def parse_labels_to_tensorflow(data, classes, images_path, labels_path):
    ...
    with open(os.path.join(labels_path, data['label_path'][0])) as json_file:
        ...
        objects = labels_data['result']['objects']
